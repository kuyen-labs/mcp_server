...
export const UPDATE_REFERRAL_CODE_MAX_USES_DESCRIPTION =
  'Grant referral capacity by setting max_uses on a code: PATCH /api/v1/referral_codes/:code with {max_uses}. Does NOT increment actual_uses or create referral_code_uses (unlike use_referral_code). Use to onboard KOLs/agencies: e.g. max_uses:0 → max_uses:50 enables 50 redemptions. Target by referral_code or affiliate_user_identifier (+ type; resolves a single owned code via GET /referral_codes). Multiple codes per affiliate require an explicit referral_code. Requires service_role project API key.' +
  PROJECT_API_KEY_HINT +
  ' dry_run then confirmed. Example dry_run: {"affiliate_user_identifier":"0xKol...","affiliate_user_identifier_type":"evm_address","max_uses":50,"dry_run":true}.';
